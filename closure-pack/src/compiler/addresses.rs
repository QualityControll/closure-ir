...
impl<'a> MlirBuilder<'a> {
    pub(crate) fn address_of_expr(&mut self, e: &Expr) -> Result<Ref, String> {
        match e {
            Expr::Argument(i) => {
                if *i < self.closure.arguments.len() {
                    let t = self.closure.arguments[*i].clone();
                    if self.dynamic {
                    ...
                    let r = self
                        ...
                        .get(ref_index)
                        .cloned()
                        .ok_or_else(|| "argument/local index out of bounds".to_string())?;
                    if r.kind == RefKind::Address {
                        Ok(r)
                    } else {
                        Err("expression is not addressable".to_string())
                    }
                }
            }
    ...
    pub(crate) fn index_address(&mut self, sequence: &Expr, index: &Expr) -> Result<Ref, String> {
        ...
        match &base.ty {
            TypeInfo::Array { element, length } => {
                ...
                self.bounds(&idx, &len)?;
                Ok(Ref {
                    name: self.gep_array_dynamic(&base.name, &base.ty, &idx),
                    ty: (**element).clone(),
                    kind: RefKind::Address,
                })
            }
            ...
            _ => Err("indexing requires an indexable sequence".to_string()),
        }
    }
}
