use crate::{expr::{Block, Statement}, types::TypeInfo};
use super::{MlirBuilder, Ref, RefKind};

impl<'a> MlirBuilder<'a> {
    pub(crate) fn statement(&mut self, s: &Statement) -> Result<(), String> {
        match s {
            Statement::Let { local, type_info, value, .. } => {
                if *local != self.local_count { return Err(format!("invalid local index {}", local)); }
                let p = self.alloca(type_info);
                let v = self.lower_expr(value, type_info)?;
                self.store(&v.name, &p, type_info);
                self.refs.push(Ref { name: p, ty: type_info.clone(), kind: RefKind::Address });
                self.local_count += 1;
            }
            Statement::Assign { local, value } => {
                let idx = self.closure.arguments.len() + *local;
                let r = self.refs.get(idx).cloned().ok_or("local index out of bounds")?;
                let v = self.lower_expr(value, &r.ty)?;
                self.store(&v.name, &r.name, &r.ty);
            }
            Statement::AssignIndex { sequence, index, value } => {
                let p = self.index_address(sequence, index)?;
                let v = self.lower_expr(value, &p.ty)?;
                self.store(&v.name, &p.name, &p.ty);
            }
            ...
            Statement::For { local, type_info, start, end, inclusive, body } => {
                ...
                let pred = if type_info.is_float() { if *inclusive { "ole" } else { "olt" } } else if type_info.is_unsigned_integer() { if *inclusive { "ule" } else { "ult" } } else if *inclusive { "sle" } else { "slt" };
                ...
                let n = self.value(); let op = if type_info.is_float() { "fadd" } else { "add" };
                self.text.push_str(&format!("    {} = llvm.{} {}, {} : {}\n", n, op, cur, one, Self::ty(type_info)));
                self.store(&n, &p, type_info); self.emit_branch(&cb); self.label(&xb);
            }
        }
        Ok(())
    }

    pub(crate) fn lower_block(&mut self, b: &Block, expected: Option<&TypeInfo>) -> Result<Option<Ref>, String> {
        for s in &b.statements { self.statement(s)?; }
        if let Some(e) = &b.result { Ok(Some(self.lower_expr(e, expected.ok_or("block result has no expected type")?)?)) }
        else if matches!(expected, Some(TypeInfo::Struct { name, fields }) if name == "()" && fields.is_empty()) { Ok(None) }
        else if expected.is_some() { Err("block has no result expression".into()) }
        else { Ok(None) }
    }
}
