...
impl<'a> MlirBuilder<'a> {
    ...
    pub(crate) fn c_float(&mut self, n: &str, ty: &str) -> String {
        let v = self.value();
        let literal = if n.contains('.') || n.contains('e') || n.contains('E') {
            n.to_string()
        } else {
            format!("{}.0", n)
        };
    ...
    pub(crate) fn gep_array_dynamic(&mut self, p: &str, t: &TypeInfo, idx: &str) -> String {
        let v = self.value();
        self.text.push_str(&format!(
            "    {} = llvm.getelementptr {}[0, {}] : (!llvm.ptr, i64) -> !llvm.ptr, {}\n",
            v,
            p,
            idx,
            Self::ty(t)
    ...
    pub(crate) fn build(mut self) -> Result<String, String> {
        ...
        let cap_struct = TypeInfo::Struct {
            name: "captures".into(),
            fields: self
                .closure
                .captures
        ...
        for f in &self.closure.external_functions {
            let args = f
                .arguments
                .iter()
                .map(Self::ty)
        ...
        self.current_terminated = false;
        for (i, t) in self.closure.captures.iter().enumerate() {
            let p = self.gep_const("%captures", &cap_struct, &[0, i]);
            let v = self.load(&p, t);
            let r = Ref {
                name: v,
                ty: t.clone(),
        ...
        for (i, t) in self.closure.arguments.iter().enumerate() {
            let v = if self.dynamic {
                ...
                let p = self.gep_raw("%args", "!llvm.ptr", &idx);
        ...
        writeln!(self.text,"    llvm.call @{}(%captures, %args, %result) : (!llvm.ptr, !llvm.ptr, !llvm.ptr) -> ()",self.name).unwrap();
        writeln!(self.text, "    llvm.return\n  }}\n}}\n").unwrap();
        Ok(self.text)
    }
}
