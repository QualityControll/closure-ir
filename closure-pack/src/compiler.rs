use crate::{
    expr::Closure,
    jit::{CompiledClosure, DynamicCompiledClosure},
    types::CompileType,
};
...
impl<'ctx> Compiler<'ctx> {
    ...
    {
        self.compile_with_libraries(closure, &[])
    }
    pub fn compile_with_libraries<Args, Ret>(
        &self,
        closure: &Closure,
        libraries: &[&str],
    ) -> Result<CompiledClosure<'ctx, Args, Ret>, String>
        ...
        Ret: CompileType + 'static,
    {
        ...
        let engine = ExecutionEngine::new(&module, 0, libraries, false, false);
        Ok(CompiledClosure::new(engine, name))
    }
    pub fn compile_captured<Args, Ret, Captures>(
        &self,
        closure: &Closure,
        captures: Captures,
    ) -> Result<CompiledClosure<'ctx, Args, Ret, Captures>, String>
    where
        Args: CompileType,
        Captures: CompileType,
        Ret: CompileType + 'static,
    {
        self.compile_captured_with_libraries(closure, captures, &[])
    }
    pub fn compile_captured_with_libraries<Args, Ret, Captures>(
        &self,
        closure: &Closure,
        captures: Captures,
        libraries: &[&str],
        ...
        Ret: CompileType + 'static,
    {
        let (module, name) = self.build_module(closure, "compiled_closure", false)?;
        let engine = ExecutionEngine::new(&module, 0, libraries, false, false);
        Ok(CompiledClosure::new_captured(engine, name, captures))
    }
    pub fn compile_dynamic(
        &self,
        closure: &Closure,
    ) -> Result<DynamicCompiledClosure<'ctx>, String> {
        self.compile_dynamic_with_libraries(closure, &[])
    }
    pub fn compile_dynamic_with_libraries(
        &self,
        closure: &Closure,
        libraries: &[&str],
    ) -> Result<DynamicCompiledClosure<'ctx>, String> {
    ...
    fn build_module(
        ...
        name: &str,
        dynamic: bool,
    ) -> Result<(Module<'ctx>, String), String> {
        ...
        let module = Module::parse(self.context, &ir)
            .ok_or_else(|| format!("failed to parse generated MLIR:\n{}", ir))?;
