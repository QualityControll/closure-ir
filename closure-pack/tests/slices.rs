...
fn test_slice_indexing_i32() {
    let compiled = compile_closure!(|values: &[i32]| -> i32 { values[0] + values[1] + values[2] });
...
fn test_slice_indexing_f64() {
    let compiled = compile_closure!(|values: &[f64]| -> f64 { values[0] * values[2] });
...
fn test_nested_slice_indexing() {
    let compiled = compile_closure!(|values: &[[i32; 2]]| -> i32 { values[0][1] + values[1][0] });
...
fn test_empty_slice_is_valid_type() {
    let compiled = compile_closure!(|values: &[i32]| -> i32 { values[0] });
    let values: &[i32] = &[42];
...
fn test_slice_last_index_is_valid() {
    let compiled = compile_closure!(|values: &[i32]| -> i32 { values[2] });
...
#[test]
fn test_slice_len() {
    let compiled = compile_closure!(|values: &[i32]| -> usize { values.len() });
...
#[test]
fn test_array_len() {
    let compiled = compile_closure!(|values: [i32; 4]| -> usize { values.len() });
