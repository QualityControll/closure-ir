...
fn dynamic_invocation_add_i32() {
    let closure = closure_pack!(|x: i32, y: i32| -> i32 { x + y });
    let context = Context::new();
    let compiler = closure_pack::Compiler::new(&context);
    let compiled = compiler
        .compile_dynamic(&closure)
        .expect("failed to compile closure");
    let result = unsafe {
        compiled
            .call(&[closure_pack::Value::I32(10), closure_pack::Value::I32(20)])
            .expect("failed to invoke closure")
    };
    assert_eq!(result, closure_pack::Value::I32(30));
}
#[test]
fn test_serialization() {
    let expr = closure_pack!(|x: i32| -> i32 { x });
