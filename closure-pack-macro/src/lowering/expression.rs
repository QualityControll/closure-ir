    ...
    match expr {
        ...
        SynExpr::MethodCall(method) => {
            if method.method != "len" {
                return Err(syn::Error::new_spanned(method, "unsupported method"));
            }
            if !method.args.is_empty() {
                return Err(syn::Error::new_spanned(method, "len() takes no arguments"));
            }
            let sequence = lower_expr(&method.receiver, arguments, locals, None)?;
            Ok(quote!(::closure_pack::Expr::Len{sequence:Box::new(#sequence)}))
        }
        SynExpr::Struct(struct_expr) => {
            structure::lower_struct(struct_expr, arguments, locals, expected_type)
        }
        SynExpr::Cast(cast) => {
            let source_type = expression_type(&cast.expr, arguments, locals).ok_or_else(|| {
                syn::Error::new_spanned(&cast.expr, "cannot infer the source type of this cast")
            })?;
            let target_type = (*cast.ty).clone();
            let value = lower_expr(&cast.expr, arguments, locals, Some(&source_type))?;
            Ok(
                quote!(::closure_pack::Expr::Cast{expr:Box::new(#value),source_type:<#source_type as ::closure_pack::CompileType>::type_info(),target_type:<#target_type as ::closure_pack::CompileType>::type_info()}),
            )
        }
        SynExpr::Index(index) => {
            ...
            Ok(
                quote!(::closure_pack::Expr::Index{sequence:Box::new(#sequence),index:Box::new(#index_expr)}),
            )
        }
...
pub(crate) fn expression_type(
    ...
    match expr {
        ...
        SynExpr::Binary(b) => expression_type(&b.left, arguments, locals)
            .or_else(|| expression_type(&b.right, arguments, locals)),
        ...
        SynExpr::Call(c) => c
            .args
            .first()
            .and_then(|a| expression_type(a, arguments, locals)),
        SynExpr::MethodCall(m) if m.method == "len" && m.args.is_empty() => {
            Some(syn::parse_quote!(usize))
        }
        SynExpr::Index(index) => {
        ...
        SynExpr::Struct(s) => Some(Type::Path(syn::TypePath {
            qself: None,
            path: s.path.clone(),
        })),
        _ => None,
