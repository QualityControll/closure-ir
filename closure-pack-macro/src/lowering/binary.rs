...
pub(crate) fn lower_binary(
    ...
    let operand_type = expression_type(&binary.left, arguments, locals)
        .or_else(|| expression_type(&binary.right, arguments, locals));

    let operand_type = operand_type.as_ref().or(expected_type);

    let lhs = lower_expr(&binary.left, arguments, locals, operand_type)?;

    let rhs = lower_expr(&binary.right, arguments, locals, operand_type)?;
    ...
    let operation = match binary.op {
        ...
        syn::BinOp::BitAnd(_) => quote! {
            ::closure_pack::Expr::BitAnd
        },

        syn::BinOp::BitOr(_) => quote! {
            ::closure_pack::Expr::BitOr
        },

        syn::BinOp::BitXor(_) => quote! {
            ::closure_pack::Expr::BitXor
        },

        syn::BinOp::Shl(_) => quote! {
            ::closure_pack::Expr::Shl
        },

        syn::BinOp::Shr(_) => quote! {
