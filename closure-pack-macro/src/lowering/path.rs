...
pub(crate) fn lower_path(
    ...
    if let Some(argument) = arguments.iter().find(|argument| &argument.name == name) {
        if argument.capture {
            let index = arguments
                ...
                .unwrap();
            return Ok(quote! {::closure_pack::Expr::Capture(#index)});
        }
        let index = arguments
            .iter()
            .filter(|a| !a.capture)
    ...
    if let Some(local) = locals.iter().rev().find(|local| &local.name == name) {
        let index = arguments.iter().filter(|a| !a.capture).count() + local.index;
        return Ok(quote! {::closure_pack::Expr::Argument(#index)});
    }
    Err(syn::Error::new_spanned(
        path,
        format!(
            "unknown closure argument, capture, or local variable `{}`",
            name
        ),
    ))
}
