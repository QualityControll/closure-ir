...
pub(crate) fn lower_if(
    ...
    let else_branch = if_expr
        .else_branch
        .as_ref()
        .ok_or_else(|| syn::Error::new_spanned(if_expr, "if expressions require an else branch"))?;

    let else_branch = match &*else_branch.1 {
        SynExpr::Block(block) => {
            lower_branch_block(&block.block, arguments, locals, expected_type)?
        }
...
fn lower_branch_block(
    ...
    let statement = block
        .stmts
        .last()
        .ok_or_else(|| syn::Error::new_spanned(block, "if branch cannot be empty"))?;
    match statement {
        Stmt::Expr(expr, _) => lower_expr(expr, arguments, locals, expected_type),
        other => Err(syn::Error::new_spanned(
            other,
            "if branches currently require a final expression",
        )),
    }
}
