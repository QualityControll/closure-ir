...
fn lower_statements(statements: &[Stmt], arguments: &[ClosureArgument], locals: &[LocalVariable], expected_type: Option<&Type>) -> syn::Result<TokenStream> {
        ...
        match statement {
            Stmt::Local(local) => {
                ...
                let type_info = match &inferred_type {
                    Some(local_type) => quote! { <#local_type as ::closure_pack::CompileType>::type_info() },
                    None => {
                        let initializer_expr = &initializer.expr;
                        let helper = match arguments.len() {
                            0 => quote! { ::closure_pack::type_info_of(|| #initializer_expr) },
                            1 => {
                                let argument = &arguments[0];
                                let name = &argument.name;
                                let ty = &argument.type_info;
                                quote! { ::closure_pack::type_info_of1(|#name: #ty| #initializer_expr) }
                            }
            ...
            Stmt::Expr(expr, _) => {
                ...
                if let syn::Expr::Assign(assign) = expr {
                    if matches!(&*assign.left, syn::Expr::Index(_)) {
                        ...
                        statement_tokens.push(quote! { ::closure_pack::Statement::AssignIndex { sequence: #sequence, index: #index, value: #value } });
                        continue;
                    }
                    ...
                    statement_tokens.push(quote! { ::closure_pack::Statement::Assign { local: #index, value: #value } });
                    continue;
                }
    ...
    Ok(quote! { ::closure_pack::Block { statements: vec![#(#statement_tokens),*], result: #result_tokens } })
}
