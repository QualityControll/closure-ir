...
impl CaptureVisitor {
 fn new(arguments:&[ClosureArgument])->Self{let mut bound=BTreeSet::new();for a in arguments{bound.insert(a.name.to_string());}Self{bound,names:BTreeSet::new()}}
}
...
fn contains_name(expr:&Expr,name:&Ident)->bool{struct V<'a>{name:&'a Ident,found:bool}impl<'ast,'a> Visit<'ast> for V<'a>{fn visit_expr_path(&mut self,node:&'ast syn::ExprPath){if node.path.segments.len()==1&&node.path.segments[0].ident==*self.name{self.found=true;}visit::visit_expr_path(self,node)}}let mut v=V{name,found:false};v.visit_expr(expr);v.found}

fn infer_in_expr(expr:&Expr,name:&Ident,arguments:&[ClosureArgument],locals:&[LocalVariable],expected:Option<&Type>)->Option<Type>{
 match expr {
  ...
  Expr::Binary(b)=>{
   let lt=expression_type(&b.left,arguments,locals);let rt=expression_type(&b.right,arguments,locals);
   if let Some(t)=lt.as_ref(){if let Some(found)=infer_in_expr(&b.right,name,arguments,locals,Some(t)){return Some(found);}}
   if let Some(t)=rt.as_ref(){if let Some(found)=infer_in_expr(&b.left,name,arguments,locals,Some(t)){return Some(found);}}
   infer_in_expr(&b.left,name,arguments,locals,expected).or_else(||infer_in_expr(&b.right,name,arguments,locals,expected))
  },
  ...
  Expr::If(i)=>infer_in_expr(&i.cond,name,arguments,locals,Some(&syn::parse_quote!(bool))).or_else(||infer_in_expr(&i.then_branch,name,arguments,locals,expected)).or_else(||i.else_branch.as_ref().and_then(|(_,e)|infer_in_expr(e,name,arguments,locals,expected))),
  _=>None,
 }
}

pub(crate) fn discover(body:&syn::Block,arguments:&[ClosureArgument])->Vec<Capture>{
 let mut locals=LocalCollector{names:BTreeSet::new()};locals.visit_block(body);
 let mut v=CaptureVisitor::new(arguments);v.bound.extend(locals.names);v.visit_block(body);
 v.names.into_iter().map(|name|Capture{name:Ident::new(&name,proc_macro2::Span::call_site()),type_info:None}).collect()
}

pub(crate) fn infer_types(body:&syn::Block,arguments:&[ClosureArgument],captures:&mut [Capture],return_type:&Type){
 let locals=Vec::new();for capture in captures{capture.type_info=infer_block(body,&capture.name,arguments,&locals,Some(return_type));}
}
fn infer_block(block:&syn::Block,name:&Ident,arguments:&[ClosureArgument],locals:&[LocalVariable],expected:Option<&Type>)->Option<Type>{
 let mut current=locals.to_vec();for stmt in &block.stmts{match stmt{syn::Stmt::Local(local)=>{let ty=match &local.pat{Pat::Type(p)=>Some((*p.ty).clone()),_=>local.init.as_ref().and_then(|i|infer_in_expr(&i.expr,name,arguments,&current,None))};if let Pat::Ident(p)=&local.pat{current.push(LocalVariable{name:p.ident.clone(),index:current.len(),type_info:ty,mutable:p.mutability.is_some()});}if let Some(i)=&local.init{if let Some(t)=infer_in_expr(&i.expr,name,arguments,&current,None){return Some(t);}}},syn::Stmt::Expr(e,_)=>if let Some(t)=infer_in_expr(e,name,arguments,&current,expected){return Some(t)},_=>{}}}expected.cloned()}
