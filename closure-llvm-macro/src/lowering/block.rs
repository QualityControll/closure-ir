...
fn lower_statements(
    ...
    let mut statements_tokens = Vec::new();
    ...
    for (position, statement) in statements.iter().enumerate() {
        ...
        match statement {
            Stmt::Local(local) => {
                ...
                let local_type = explicit_type.or_else(|| expression_type(&initializer.expr, arguments, &current_locals)).ok_or_else(|| {
                    syn::Error::new_spanned(&initializer.expr, "cannot infer local variable type")
                })?;

                let value = lower_expr(&initializer.expr, arguments, &current_locals, Some(&local_type))?;
                let index = current_locals.len();
                let type_info = quote! {
                    <#local_type as ::closure_llvm::CompileType>::type_info()
                };

                statements_tokens.push(quote! {
                    ::closure_llvm::Statement::Let {
                        local: #index,
                        type_info: #type_info,
                        value: #value,
                        mutable: #mutable,
                ...
                current_locals.push(LocalVariable {
                    name,
                    index,
                    type_info: Some(local_type),
                    mutable,
                });
            }
    ...
    let result = result.ok_or_else(|| {
        syn::Error::new(proc_macro2::Span::call_site(), "closure block must end with an expression")
    })?;

    Ok(quote! {
        ::closure_llvm::Block {
            statements: vec![#(#statements_tokens),*],
            result: Some(#result),
        }
    })
}
...
fn assignment_target<'a>(
    ...
    let local = locals.iter().rev().find(|local| &local.name == name).ok_or_else(|| {
