...
mod tests {
    ...
    #[test]
    fn test_multiple_mutable_assignments() {
        let compiled = compile_closure!(|x: i32| -> i32 {
            let mut y = x;
            y = y + 5;
            y = y * 2;
        ...
        assert_eq!(call!(compiled, 4), 15);
    }

    #[test]
    fn test_annotated_let_mut_assignment() {
        let compiled = compile_closure!(|x: i32| -> i32 {
            let mut y: i32 = x + 1;
            y = y + 10;
            y
    ...
    #[test]
    fn test_mutable_variable_can_be_used_by_other_locals() {
        let compiled = compile_closure!(|x: i32| -> i32 {
            let mut y = x + 1;
            let z = y * 2;
            y = z + 3;
