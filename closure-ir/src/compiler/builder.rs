...
impl<'a> MlirBuilder<'a> {
    ...
    pub(crate) fn c_bool(&mut self,b:bool)->String { let v=self.value();self.text.push_str(&format!("    {} = llvm.mlir.constant({} : i1) : i1\n",v,b));v }
    pub(crate) fn c_int(&mut self,n:&str,ty:&str)->String { let v=self.value();self.text.push_str(&format!("    {} = llvm.mlir.constant({} : {}) : {}\n",v,n,ty,ty));v }
    pub(crate) fn c_float(&mut self,n:&str,ty:&str)->String { let v=self.value();self.text.push_str(&format!("    {} = llvm.mlir.constant({} : {}) : {}\n",v,n,ty,ty));v }
    ...
    pub(crate) fn build(mut self)->Result<String,String> { let arg_struct=TypeInfo::Struct{name:"args".into(),fields:self.closure.arguments.iter().enumerate().map(|(i,t)|crate::types::FieldInfo{name:i.to_string(),type_info:t.clone()}).collect()};writeln!(self.text,"module {{").unwrap();writeln!(self.text,"  llvm.func @{}(%args: !llvm.ptr, %result: !llvm.ptr) {{",self.name).unwrap();
        // The entry block of a region with function arguments is implicit in MLIR.
        // Emitting `^entry:` here is invalid because the llvm.func region has
        // named entry-block arguments (%args and %result).
