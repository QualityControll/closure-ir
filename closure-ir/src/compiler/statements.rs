    ...
    pub(crate) fn statement(&mut self, s: &Statement) -> Result<(), String> {
        match s {
            ...
            Statement::While { condition, body } => {
                let cb = self.block("while_cond");
                let bb = self.block("while_body");
                let xb = self.block("while_exit");
                self.emit_branch(&cb);
                self.label(&cb);
                let c = self.lower_expr(condition, &TypeInfo::Bool)?;
                self.emit_cond(&c.name, &bb, &xb);
                self.label(&bb);
            ...
            Statement::For { local, type_info, start, end, inclusive, body } => {
                ...
                let cmp = self.value();
                let pred = if type_info.is_float() {
                    if *inclusive { "ole" } else { "olt" }
                } else if type_info.is_unsigned_integer() {
                    if *inclusive { "ule" } else { "ult" }
                } else if *inclusive {
                    "sle"
                } else {
                    "slt"
