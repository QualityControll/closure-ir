...
impl<'ctx> Compiler<'ctx> {
    ...
    fn build_argument_pointers(&self, context: &'ctx Context, builder: &Builder<'ctx>, argument_pointer: PointerValue<'ctx>, argument_types: &[TypeInfo]) -> Result<Vec<PointerValue<'ctx>>, String> {
        if argument_types.is_empty() { return Ok(Vec::new()); }
        let field_types = argument_types.iter().map(|type_info| llvm_type(context, type_info)).collect::<Result<Vec<_>, _>>()?;
        ...
        for index in 0..argument_types.len() {
            let pointer = builder.build_struct_gep(tuple_type, argument_pointer, index as u32, &format!("arg{}_ptr", index))
    ...
    fn build_dynamic_argument_pointers(&self, context: &'ctx Context, builder: &Builder<'ctx>, argument_array: PointerValue<'ctx>, argument_types: &[TypeInfo]) -> Result<Vec<PointerValue<'ctx>>, String> {
        ...
        for index in 0..argument_types.len() {
            ...
            let pointer = unsafe { builder.build_gep(pointer_type, argument_array, &[index_value], &format!("dynamic_arg{}_slot", index)) }
                .map_err(|error| format!("failed to build dynamic argument GEP: {:?}", error))?;
            let value = builder.build_load(pointer_type, pointer, &format!("dynamic_arg{}_ptr", index))
                .map_err(|error| format!("failed to load dynamic argument {} pointer: {:?}", index, error))?.into_pointer_value();
            result.push(value);
        }
        Ok(result)
    }
...
pub(crate) fn llvm_type<'ctx>(context: &'ctx Context, type_info: &TypeInfo) -> Result<BasicTypeEnum<'ctx>, String> {
    match type_info {
        ...
        TypeInfo::Struct { fields, .. } => {
            let field_types = fields.iter().map(|field| llvm_type(context, &field.type_info)).collect::<Result<Vec<_>, _>>()?;
