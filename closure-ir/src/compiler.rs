...
impl<'ctx> Compiler<'ctx> {
    ...
    fn generate_function(&self, context: &'ctx Context, module: &Module<'ctx>, function_name: &str, closure: &Closure) -> Result<(), String> {
        ...
        let arguments = self.build_argument_pointers(context, &builder, argument_pointer, &closure.arguments)?; let value = lower_closure_block(context, &builder, function, &arguments, &closure.arguments, &closure.return_type, &closure.body)?;
    ...
    fn generate_dynamic_function(&self, context: &'ctx Context, module: &Module<'ctx>, function_name: &str, closure: &Closure) -> Result<(), String> {
        ...
        let arguments = self.build_dynamic_argument_pointers(context, &builder, argument_array, &closure.arguments)?; let value = lower_closure_block(context, &builder, function, &arguments, &closure.arguments, &closure.return_type, &closure.body)?;
