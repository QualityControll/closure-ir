...
pub enum Intrinsic {
    Sqrt, Abs, Min, Max, Floor, Ceil, Round, Sin, Cos, Tan, Exp, Log, Pow,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub enum Expr {
    Argument(usize), Constant(Value), Field { object: Box<Expr>, name: String }, Tuple { elements: Vec<Expr> },
    ...
    And { lhs: Box<Expr>, rhs: Box<Expr> }, Or { lhs: Box<Expr>, rhs: Box<Expr> }, BitAnd { lhs: Box<Expr>, rhs: Box<Expr> }, BitOr { lhs: Box<Expr>, rhs: Box<Expr> }, BitXor { lhs: Box<Expr>, rhs: Box<Expr> }, Shl { lhs: Box<Expr>, rhs: Box<Expr> }, Shr { lhs: Box<Expr>, rhs: Box<Expr> },
