...
impl<'a,T:CompileType> CompileType for &'a [T] {fn type_info()->TypeInfo{TypeInfo::Slice{element:Box::new(T::type_info())}} fn llvm_type<'ctx>(c:&'ctx Context)->BasicTypeEnum<'ctx>{c.struct_type(&[c.ptr_type(inkwell::AddressSpace::default()).into(),c.i64_type().into()],false).into()}}
impl<'a,T:CompileType> CompileType for &'a mut [T] {fn type_info()->TypeInfo{TypeInfo::Slice{element:Box::new(T::type_info())}} fn llvm_type<'ctx>(c:&'ctx Context)->BasicTypeEnum<'ctx>{c.struct_type(&[c.ptr_type(inkwell::AddressSpace::default()).into(),c.i64_type().into()],false).into()}}
