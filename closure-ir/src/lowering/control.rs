...
impl<'ctx> Lowering {
    pub(crate) fn lower_if_else(
        ...
        builder.build_conditional_branch(condition, then_block, else_block)
            .map_err(|error| format!("failed to build conditional branch: {:?}", error))?;

        builder.position_at_end(then_block);
        let then_value = self.materialize_value(context, builder,
            self.lower_expr(context, builder, function, arguments, argument_types, expected_type, then_branch)?)?;
        let then_end = builder.get_insert_block().ok_or_else(|| "missing then block".to_string())?;
        if then_end.get_terminator().is_none() {
            builder.build_unconditional_branch(merge_block)
                .map_err(|error| format!("failed to branch from then block: {:?}", error))?;
        }

        builder.position_at_end(else_block);
        let else_value = self.materialize_value(context, builder,
            self.lower_expr(context, builder, function, arguments, argument_types, expected_type, else_branch)?)?;
        let else_end = builder.get_insert_block().ok_or_else(|| "missing else block".to_string())?;
        if else_end.get_terminator().is_none() {
            builder.build_unconditional_branch(merge_block)
                .map_err(|error| format!("failed to branch from else block: {:?}", error))?;
        }

        builder.position_at_end(merge_block);
        let phi = builder.build_phi(crate::compiler::llvm_type(context, expected_type)?, "if_result")
            .map_err(|error| format!("failed to build if/else PHI: {:?}", error))?;
