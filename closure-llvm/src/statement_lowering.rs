...
pub(crate) fn lower_closure_block<'ctx>(
    ...
    let (_, _, value) = lower_block(context, builder, function, arguments, argument_types, block, Some(return_type))?;
...
fn lower_block<'ctx>(
    ...
    for statement in &block.statements {
        match statement {
            Statement::Let { local, type_info, value, .. } => {
                let expected_local = pointers.len().saturating_sub(arguments.len());
            ...
            Statement::While { condition, body } => {
                ...
                let _ = lower_block(context, builder, function, &pointers, &types, body, None)?;
    ...
    let result = match (&block.result, expected_result_type) {
        ...
        (Some(expr), None) => return Err("block result has no expected type".to_string()),
