    ...
    DynamicCompiledClosure<'ctx>
{
    ...
    pub unsafe fn call(
        ...
        let function =
            self.engine
                .get_function::<DynamicJitFn>(
                    &self.function_name,
                )
                .map_err(|error| {
                    format!(
                        "failed to get dynamic JIT function: {:?}",
                        error
                    )
                })?;
        ...
        // Argument storage
        //
        // These are deliberately kept alive until after the
        // JIT call returns.
        // ----------------------------------------------------
        ...
        // Keep argument storage alive through the call.
        //
        // The explicit reference prevents future refactoring
        // from accidentally dropping it too early.
        std::hint::black_box(
            &argument_storage
        );
...
fn bytes_to_value(
    ...
    unsafe {
        match type_info {
            ...
            _ =>
                Err(
                    "dynamic result decoding for this type \
...
fn value_size(
    ...
    match type_info {
        ...
        _ =>
            Err(
                "dynamic size calculation for this type \
                 is not implemented yet"
                    .to_string()
            ),
    }
...
fn value_alignment(
    ...
    match type_info {
        TypeInfo::Bool =>
            Ok(std::mem::align_of::<bool>()),

        TypeInfo::I8 =>
            Ok(std::mem::align_of::<i8>()),

        TypeInfo::I16 =>
            Ok(std::mem::align_of::<i16>()),

        TypeInfo::I32 =>
            Ok(std::mem::align_of::<i32>()),

        TypeInfo::I64 =>
            Ok(std::mem::align_of::<i64>()),

        TypeInfo::I128 =>
            Ok(std::mem::align_of::<i128>()),

        TypeInfo::U8 =>
            Ok(std::mem::align_of::<u8>()),

        TypeInfo::U16 =>
            Ok(std::mem::align_of::<u16>()),

        TypeInfo::U32 =>
            Ok(std::mem::align_of::<u32>()),

        TypeInfo::U64 =>
            Ok(std::mem::align_of::<u64>()),

        TypeInfo::U128 =>
            Ok(std::mem::align_of::<u128>()),

        TypeInfo::F32 =>
            Ok(std::mem::align_of::<f32>()),

        TypeInfo::F64 =>
            Ok(std::mem::align_of::<f64>()),

        _ =>
            Err(
                "dynamic alignment calculation for this type \
                 is not implemented yet"
                    .to_string()
            ),
    }
